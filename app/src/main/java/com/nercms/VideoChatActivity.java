...
public class VideoChatActivity extends AppCompatActivity {
    ...
    public void doStart() {
        ...
        if (mCamera == null) {
            ...
            //摄像头设置，预览视频
            mCamera = Camera.open(Camera.CameraInfo.CAMERA_FACING_FRONT); //实例化摄像头类对象  0为后置 1为前置
            Camera.Parameters p = mCamera.getParameters(); //将摄像头参数传入p中
            printPreviewSizes(p);
            printSupportedStringList("SupportedFlashModes", p.getSupportedFlashModes());
            printSupportedStringList("SupportedFocusModes", p.getSupportedFocusModes());
            printSupportedIntegerList("SupportedPreviewFrameRates", p.getSupportedPreviewFrameRates());
            printSupportedStringList("SupportedSceneModes", p.getSupportedSceneModes());
            printSupportedStringList("SupportedWhiteBalance", p.getSupportedWhiteBalance());
            printSupportedIntegerList("SupportedPreviewFormats", p.getSupportedPreviewFormats());
            ...
            p.setPreviewFormat(ImageFormat.NV21);//0x11 = 17
    ...
    private void printSupportedStringList(String title, List<String> list) {
        Log.e(TAG, title + "###########################################");
        int i = 1;
        for (String s : list) {
            Log.e(TAG, title + " " + i + " " + s);
    ...
    private void printSupportedIntegerList(String title, List<Integer> list) {
        Log.e(TAG, title + "###########################################");
        int i = 1;
        for (int s : list) {
            Log.e(TAG, title + " " + i + " " + s);
    ...
    private void printPreviewSizes(Camera.Parameters p) {
        List<Camera.Size> supList = p.getSupportedPreviewSizes();
        for (Camera.Size size : supList) {
